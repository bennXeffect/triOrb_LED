...
unsigned char LEDcol[5] = {0, 0, 255, 0xFF, 0xFF}; // the RGB values and the state of blink
...
// Raspberry Piからのリクエスト受信時に呼ばれる関数
void requestEvent()
{
  Wire.write(42); // レスポンスとしてデータを送信 (例: 42)
}

// Raspberry Piからのデータ受信時に呼ばれる関数
...
{
  Wire.begin(I2C_ADR);
  Wire.onRequest(requestEvent); // リクエスト受信時のコールバック関数を登録
  Wire.onReceive(receiveEvent); // 受信時のコールバック関数を登録
  Serial.begin(115200);
...
void loop()
{
  ...
  stateCode = (((uint16_t)LEDcol[3]) << 8) + ((uint16_t)LEDcol[4]);

  for (int a = 0; a < NUM_GROUPS; a++)
  { // put the colors
    if (bitRead(stateCode, 0) == 1 && bitRead(stateCode, a + 1) == 1)
    { // the groups that need blinking state
      blinkState[a] = 1;
    }
    if (bitRead(stateCode, 0) == 0 && bitRead(stateCode, a + 1) == 1)
    { // the groups that need NO blinking state
      blinkState[a] = 0;
    }
    if (bitRead(stateCode, a + 1) == 1)
    { // determine if the group needs to change values
      // if we are here, we need to set the incoming RGB color into the the corresponding LED
      for (int c = 0; c < 3; c++)
      { // changinng the color values
